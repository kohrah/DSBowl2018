...
# Author: albu
# Original gist: https://github.com/asanakoy/kaggle_carvana_segmentation/blob/master/albu/src/transforms.py
...
class ShiftScaleRotate:
    ...
    def __call__(self, img, mask=None):
        ...
        if random.random() < self.prob:
            ...
            img_ = cv2.warpPerspective(img_, mat, (width, height),
                                      flags=cv2.INTER_LINEAR,
                                      borderMode=cv2.BORDER_REFLECT_101)
            if mask is not None:
                mask_ = cv2.warpPerspective(mask_, mat, (width, height),
                                           flags=cv2.INTER_LINEAR,
                                           borderMode=cv2.BORDER_REFLECT_101)
...
class RandomBrightness:
    def __init__(self, limit=0.2, prob=0.5): #limit was 0.1
        self.limit = limit
        self.prob = prob
    ...
    def __call__(self, img):
        ...
        maxval = 0
        if random.random() < self.prob:
            alpha = 1.0 + self.limit*random.uniform(-1, 1) # was (-1, 1)
            maxval = np.max(img_[..., :3])
            dtype = img_.dtype
            img_[..., :3] = clip(alpha * img_[...,:3], dtype, maxval)
        return img_
...
class RandomSaturation:
    ...
    def __call__(self, img):
        ...
        if random.random() < self.prob:
            ...
            img_[..., :3] = alpha * img_[..., :3] + (1.0 - alpha) * gray
            img_[..., :3] = clip(img_[..., :3], dtype, maxval)
        return img_
